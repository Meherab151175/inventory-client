...
const ManageProducts = () => {
    ...
    useEffect(() => {
        axiosFetch.get('/products-manage')
    ...
    const handleApprove = (id) => {
        axiosSecure.put(`/change-status/${id}`, { status: 'approved' })
            .then(res => {
                ...
                setProducts(products.map(cls => cls._id == id ? { ...cls, status: 'approved' } : cls))
            })
            .catch(err => console.log(err))
    }
            ...
            preConfirm: async (text) => {
                try {
                    ...
                    if (res.data.modifiedCount > 0) {
                        setProducts(products.map(cls => cls._id == id ? { ...cls, status: 'rejected' } : cls))
                    }
                    return res.data
                } catch (error) {
    ...
    return (
        <div>
            <h1 className='text-4xl text-secondary font-bold text-center my-10'>Manage <span className='text-black'>Products</span></h1>
                        ...
                        <div className="inline-block min-w-full py-2 sm:px-6 lg:px-8">
                            <div className="overflow-hidden">
                                <table className="min-w-full text-left text-sm font-light">
                                    ...
                                    <tbody>
                                        {
                                            products.length == 0 ? <tr><td colSpan='6' className='text-center text-2xl font-bold'>No Products Found</td></tr> :
                                                paginatedData.map((cls, idx) => <tr
                                                    ...
                                                    <td className="whitespace-nowrap px-6 py-4">{cls.sellerName}</td>
                                                    <td className="whitespace-nowrap px-6 py-4">
