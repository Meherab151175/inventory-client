...
const CheckoutPayment = ({ price , cartItm }) => {
    ...
    useEffect(() => {
        axiosSecure.get(`/cart/${currentUser?.email}`)
            .then((res) => {
                // SET PRODUCTS ID IN STATE
                const productsId = res?.data?.map(item => item._id);
                setCart(productsId)
            })
            .catch((err) => {
                console.log(err)
        ...
        if (confirmError) {
            ...
            if (paymentIntent.status === 'succeeded') {
                ...
                const data = {
                    ...
                    userName,
                    userEmail,
                    productsId : cartItm ? [cartItm] : cart, 
                    date : new Date()
                }
                ...
                fetch(URL, {
                    ...
                    .then(res => {
                        ...
                        if (res?.deletedResult?.deletedCount > 0 && res.paymentResult.insertedId && res.updatedResult.modifiedCount > 0) {
                            setSucceeded('Payment Successful , You can now access your products')
                        }
                        else {
